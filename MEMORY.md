...
- Export JSON is `schemaVersion: 2`: import accepts ONLY 2 (v1 rejected with "schemaVersion … not supported"), per-version `isActive` + template `isActive` flags preserved exactly, no skip/collapse on import. Known gap: bulk-zip `_summary.json` still serializes `schemaVersion: 1`.
